...
function ReviewServer(options)
{
	...
	function reviewPullRequest(card)
	{
		...
		{
			var destination;
			if (card.idList == options.inProgress && comments.length)
			{
				destination = options.underReview;
			}
			var score = computeScore(comments);
			if (score >= 2)
			{
				destination = options.reviewed;
			}
			else if (score < 0)
			{
				destination = options.blocked;
			}
			board.moveCard(card.id, destination, function(error)
			{
				if (error)
				{
					console.log('Could not move card %s: %s', card.name, error);
				}
				var newName = getNameWithScore(card.name, score);
				board.renameCard(card.id, newName, function(error)
				{
					if (error)
					{
						console.log('Could not rename card to %s: %s', newName, error);
					}
				});
			});
		});
	...
	function computeScore(comments)
	{
		...
		{
			var author = comment.author.username;
			if (author in voted)
			{
				return;
			}
			...
			{
				var veto = parseFloat(comment.text.substringFrom('-'));
				vetos += veto;
				voted[author] = -veto;
			}
			if (comment.text.contains('+'))
			{
				var vote = parseFloat(comment.text.substringFrom('+'));
				score += vote;
				voted[author] = vote;
			}
		});
		console.log('Score: %s, vetos: %s, tally: %s', score, vetos, JSON.stringify(voted, null, '\t'));
		if (vetos > 0)
		{
			return -vetos;
		}
	...
	{
		var original = name;
		if (score.contains(' (') && score.endsWith(')'))
		{
