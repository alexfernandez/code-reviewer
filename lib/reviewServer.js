 ...
 *	- secret: secret value to access the server (must be in URL).
...
function ReviewServer(options)
{
	...
	self.start = function(callback)
	{
		...
		server.on('error', function(error)
		{
			...
			{
				return callback('Port ' + port + ' in use, please free it and retry again');
			}
			return callback('Could not start server on port ' + port + ': ' + error);
		});
	...
	function end(url, body, response)
	{
		...
		{
			log.info('Received request to invalid URL %s', url);
	...
	/**
	 * Create comment in Trello card.
	 */
	self.createComment = function(info)
	{
		var sender = info.sender.login;
		// sometimes we receive an issue instead of a pull request
		info.pull_request = info.pull_request || info.issue;
		if (!info.pull_request)
		{
			return log.warning('No pull_request or issue in message: %s', JSON.stringify(info, null, '\t'));
		}
		var title = getTitle(info);
		var body = info.sender.login + ': ' + info.comment.body + '\n\n[View on GitHub](' + info.comment.html_url + ')';
		if (!info.comment.body.contains('+1') && !info.comment.body.contains('-1'))
		{
			return log.info('Comment created by %s on %s: %s', sender, title, body);
		}
		var card = board.findCard(title.substringUpTo(']') + ']');
		if (!card)
		{
	...
	function getTitle(info)
	{
		...
		return '[' + repoName + ': #' + info.pull_request.number + '] ' + info.pull_request.title;
	}
	...
	{
		return log.info('Received action %s', info.action);
	};
...
function computeScore(comments)
{
	...
	{
		var text = comment.text.substringUpToLast('\n');
		if (!text.contains(':'))
		{
			return;
		}
