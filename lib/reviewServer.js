...
function ReviewServer(options)
{
	...
	function analyze(data)
	{
		...
		{
			log.info('Invalid message received: %s', data.substring(0, 20));
			return;
		}
		if (!json.action || !json.repository || !json.pull_request)
		{
			log.info('Missing action/repository/pull_request in message: %s / %s / %s', json.action, json.repository, json.pull_request);
		}
		if (!json.repository)
		{
			log.info('No action in message: %s', data.substring(0, 20));
		}
		var action = json.action;
		var repoName = json.repository.name;
		var title = '[' + repoName + '] ' + json.pull_request.title;
		var description = json.pull_request.body;
		var username = json.pull_request.user.login;
		if (action == 'opened')
		{
