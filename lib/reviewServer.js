...
function ReviewServer(options)
{
	...
	function analyze(info)
	{
		...
		if (action == 'opened')
		{
			return openPullRequest(info);
		}
		if (action == 'closed')
		{
			return closePullRequest(info);
		}
		handleUnknownAction(info);
	}
	...
	function closePullRequest(info)
	{
		var title = getTitle(info);
		if (!title)
		{
		...
		board.moveCard(title, options.cancelled, function(error)
		{
			if (error)
			{
				return log.error('Could not move card %s: %s', title, error);
			}
			return log.info('Moved card for %s to cancelled', title);
		});
	}
	...
	function handleUnknownAction(info)
	{
		...
		fs.write(filename, JSON.stringify(info, null, '\t'), function(error)
		{
			if (error)
			{
