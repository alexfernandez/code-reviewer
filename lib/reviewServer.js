...
function ReviewServer(options)
{
	...
	function analyze(info)
	{
		...
		if (action == 'opened')
		{
			board.createCard(title, description, options.inProgress, options, function(error)
			{
				if (error)
				{
	...
	function createComment(info)
	{
		...
		board.readListCards(options.underReview, options, function(error, cards)
		{
			if (error)
			{
			...
			board.addComment(cardId, body, options, function(error)
			{
				if (error)
				{
