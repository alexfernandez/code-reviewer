...
exports.createCard = function(name, description, listId, options, callback)
{
	var variables = {
		name: name,
...
{
	exports.createCard('name', 'description', credentials.inProgress, credentials, function(error)
	{
...
exports.readListCards = function(listId, options, callback)
{
	var variables = {
		token: options.token,
		key: options.key,
...
exports.addComment = function(cardId, text, options, callback)
{
	var variables = {
		text: text,
		token: options.token,
...
{
	exports.readListCards(credentials.inProgress, credentials, function(error, cards)
	{
		testing.check(error, 'Could not read list', callback);
		testing.assert(cards.length, 'No cards in inProgress', callback);
		exports.addComment(cards[0].id, 'Test comment.', credentials, function(error)
		{
