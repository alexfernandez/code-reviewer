	...
	function getListMaker(key, defaultName, board)
	{
		return function(callback)
		{
			...
			if (id)
			{
				...
				if (!byId.length)
				{
					return callback('List ' + key + ' with id ' + id + ' in configuration not found');
				}
				log.info('Using preconfigured %s with id %s', key, id);
			}
			else
			{
				...
				id = byName[0].id;
				log.info('Using list with default name ' + defaultName + ' and id ' + id);
			}
			return readList(id, callback);
		};
	}
